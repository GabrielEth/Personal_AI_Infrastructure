...
**Agent System** — PAI uses Task tool subagent_types (Architect, Engineer, Intern, Explore, etc.) for delegating work to specialized agents. Intern agents handle parallel grunt work. The spotcheck pattern verifies parallel work.
...
**System Management** — PAI manages its own integrity, security, and documentation. This includes: integrity audits (checking for broken references), secret scanning (TruffleHog detection), privacy validation (ensuring USER/WORK content isolation), cross-repo validation (private vs public separation), and documentation updates (MEMORY/PAISYSTEMUPDATES/).
...
| Directory | Purpose |
|-----------|---------|
| **skills/** | Skill modules (CORE, Research, Browser, etc.) |
